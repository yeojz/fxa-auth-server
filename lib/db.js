...
zlib.gzipAsync = P.promisify(zlib.gzip)
zlib.gunzipAsync = P.promisify(zlib.gunzip)
...
module.exports = (
  ...
  DB.prototype.updateSessionToken = function (token, geo) {
    ...
    return geo
      ...
      .catch(() => {})
      .then(() => redis.update(uid, packedTokens => {
        return P.all([
          unpackTokensFromRedis(packedTokens),
          packTokenForRedis({
        ...
        ]).spread((sessionTokens, packedToken) => {
          sessionTokens[id] = packedToken
  ...
  // Reduce redis memory usage by gzipping tokens. Per-token rather than per-user
  // because we need to co-exist with the existing data. Returns a promise.
  function packTokenForRedis (token) {
    return zlib.gzipAsync(JSON.stringify(token))
      .then(result => result.toString('base64'))
  }

  // Sanely unpack both the gzipped and raw formats from redis. Returns a promise.
  function unpackTokensFromRedis (tokens) {
    return P.resolve()
      .then(() => {
        ...
        return P.all(tokenIds.map(tokenId => {
          const packedToken = tokens[tokenId]

          if (typeof packedToken === 'string') {
            return zlib.gunzipAsync(Buffer.from(packedToken, 'base64'))
              .then(stringifiedToken => JSON.parse(stringifiedToken))
          } else {
            return packedToken
          }
        })).then(unpackedTokens => unpackedTokens.reduce((result, token, index) => {
          result[tokenIds[index]] = token
          return result
        }, {}))
      })
  }
