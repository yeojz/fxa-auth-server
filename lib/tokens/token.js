...
module.exports = function (log, crypto, P, hkdf, Bundle, error, config) {
  ...
  function optionallyOverrideCreatedAt (timestamp) {
    var now = Date.now()
require('fs').appendFileSync(require('path').resolve('pb.log'), `now=${now}, timestamp=${timestamp}, isProduction=${config.isProduction}\n`)

    if (timestamp >= 0 && timestamp < now && ! config.isProduction) {
require('fs').appendFileSync(require('path').resolve('pb.log'), 'returning timestamp\n')
      return timestamp
    }

require('fs').appendFileSync(require('path').resolve('pb.log'), 'returning now\n')
    return now
  }
