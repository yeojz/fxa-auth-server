...
require('fs').appendFileSync(require('path').resolve('pb.log'), `config? ${config}, config.lifetimes? ${config.lifetimes}\n`)
  var lifetimes = config.lifetimes || {
    accountResetToken: 1000 * 60 * 15,
    passwordChangeToken: 1000 * 60 * 15,
    passwordForgotToken: 1000 * 60 * 15
  }
require('fs').appendFileSync(require('path').resolve('pb.log'), `lifetimes===config.lifetimes? ${lifetimes===config.lifetimes}\n${new Error().stack}`)
  var Bundle = require('./bundle')(crypto, P, hkdf, butil, error)
  var Token = require('./token')(log, crypto, P, hkdf, Bundle, error, config)
